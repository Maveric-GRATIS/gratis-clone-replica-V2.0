...
const placeholderWinners: Winner[] = [
  {
    ...
    creationDescription: 'A mysterious blend that keeps you guessing with every sip.',
    creationFlavor: 'Mystery Citrus Fusion',
    creationImageUrl: '/lovable-uploads/9ac6ca6a-c33b-407e-ae9d-d71edd6d1a95.png',
    impactProject: 'Street Art Programs',
    impactAmount: 4500,
    seriesNumber: 3,
    editionSize: 300,
    winnerQuote: "I wanted to create something that represents the unpredictable energy of street culture.",
    featured: true,
  ...
  useEffect(() => {
    const fetchWinners = async () => {
      ...
      try {
        ...
        const fetchedWinners = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Winner));
        
        setWinners(fetchedWinners.length > 0 ? fetchedWinners : placeholderWinners);
      } catch (error) {
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-yellow-500 font-bold text-sm tracking-wider uppercase">Hall of Fame</span>
          </div>
            ...
            >
              {winner.featured && (
                <div className="absolute top-4 right-4 z-20">
                  <Badge className="bg-yellow-500 text-black font-bold"><Sparkles className="w-3 h-3 mr-1" />Featured</Badge>
                </div>
              )}
              ...
              <div className="p-6 space-y-4">
                <div>
                  <h3 className="text-2xl font-black tracking-tight text-foreground">{winner.creationName || 'Unnamed Creation'}</h3>
                  <p className="text-sm text-primary font-medium">{winner.creationFlavor}</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-foreground">{winner.winnerName}</p>
                    {winner.winnerLocation && (
                      <p className="text-xs text-muted-foreground flex items-center gap-1"><MapPin className="w-3 h-3" />{winner.winnerLocation}</p>
                    )}
                  </div>
                </div>
                ...
                {winner.winnerQuote && (
                  <div className="bg-muted/50 rounded-lg p-4 relative">
                    ...
                    <p className="text-sm italic text-muted-foreground pl-4">"{winner.winnerQuote}"</p>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border/50">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-sm">
                      <span className="font-bold text-foreground">{winner.impactAmount ? formatEuro(winner.impactAmount) : '€0'}</span>
                ...
                {winner.impactProject && (
                  <div className="text-xs text-center">
                    ...
                    <span className="font-semibold text-primary">{winner.impactProject}</span>
                  </div>
                )}

                {winner.editionSize && (
                  <div className="text-center">
                    <Badge variant="outline" className="text-xs">Limited Edition: {winner.editionSize} Units</Badge>
                  </div>
                )}
              </div>
            </Card>
        ...
        <div className="text-center mt-16">
          <div className="inline-flex flex-col items-center gap-4 p-8 rounded-2xl bg-gradient-to-br from-yellow-500/10 to-orange-500/10 border border-yellow-500/20">
            ...
            <p className="text-muted-foreground max-w-md">Find a golden cap in any F.U. bottle and you could be next.</p>
            <Badge className="bg-yellow-500 text-black text-lg py-2 px-6 hover:bg-yellow-400 transition-colors">1 in every series has the golden cap</Badge>
          </div>
        </div>
      </div>
    </section>
